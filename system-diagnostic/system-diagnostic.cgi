...
case "$ACTION" in
    ...
    "ping")
        ...
        return_success "pong"
        ;;
# Adicionar no final do case statement do CGI:

        "storage-test")
            log_debug "Executando teste de armazenamento"
            ...
            if output=$(timeout 120 bash -c "
                ...
                # Verificar inodes
                echo '📁 Verificação de inodes:'
                df -i | awk 'NR>1 && \$5 != \"-\" {gsub(/%/, \"\", \$5); if (\$5 > 95) print \"❌ CRÍTICO: \" \$0; else if (\$5 > 80) print \"⚠️  AVISO: \" \$0; else print \"✅ OK: \" \$0}'
        ...
        "network-test")
            log_debug "Executando teste de rede"
        ...
        "services-test")
            log_debug "Executando teste de serviços"
            
            if output=$(timeout 30 bash -c "
                echo '⚙️  TESTE DE SERVIÇOS'
                echo '===================='
                echo ''
                ...
                if command -v docker >/dev/null 2>&1; then
                    if systemctl is-active --quiet docker 2>/dev/null; then
                        ...
                        echo \"🐳 Docker: Ativo (\$container_count containers)\"
                    else
                        echo \"🐳 Docker: Instalado mas inativo\"
                    fi
                fi
        ...
        "system-test")
            log_debug "Executando teste de sistema"
            
            if output=$(timeout 30 bash -c "
                echo '🖥️  TESTE DE SISTEMA'
                echo '=================='
                echo ''
                ...
                load_avg=\$(uptime | awk '{print \$(NF-2)}' | sed 's/,//')
                cpu_cores=\$(nproc)
                echo \"📊 Carga: \$load_avg (CPUs: \$cpu_cores)\"
                
                # Memória
                mem_info=\$(free -h | awk 'NR==2{printf \"Total: %s, Usado: %s (%s), Livre: %s\", \$2, \$3, \$5, \$4}')
                ...
                zombies=\$(ps aux | awk '\$8 ~ /^Z/ { count++ } END { print count+0 }')
                if [ \"\$zombies\" -gt 0 ]; then
                    echo \"⚠️  Processos zumbi: \$zombies\"
                else
                    echo \"✅ Processos zumbi: 0\"
                fi
